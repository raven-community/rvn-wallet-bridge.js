...
class RVNWalletBridge {
    ...
    getAssetAddress(changeType, index, assetName) {
        return this.getAddresses(changeType, index, 1, assetName)
            .then((addresses) => {
    ...
    getAssetAddressIndex(changeType, assetName) {
        const walletProvider = this.checkWalletProvider();
        return walletProvider.getAddressIndex(changeType, assetName)
            .then((index) => {
    ...
    getAddresses(changeType, startIndex, size, assetName) {
        if (startIndex) {
        ...
        const walletProvider = this.checkWalletProvider();
        return walletProvider.getAddresses(changeType, size || 1, startIndex, assetName)
            .then((addresses) => {
    ...
    async getRedeemScript(p2shAddress, assetName) {
        ...
        const redeemScripts = await this.getRedeemScripts(assetName);
        return redeemScripts.find((script) => this.toAddressFromScript(script) === p2shAddress);
    }
    async getRedeemScripts(assetName) {
        const walletProvider = this.checkWalletProvider();
        const redeemScripts = await walletProvider.getRedeemScripts(assetName)
            .catch((e) => { throw new ProviderException_1.default(e); });
    ...
    async addRedeemScript(redeemScript, assetName) {
        if (redeemScript.length < 1) {
            throw new IllegalArgumentException_1.default("The redeemScript cannot be empty.");
        }
        const walletProvider = this.checkWalletProvider();
        const result = await walletProvider.addRedeemScript(redeemScript, assetName)
            .catch((e) => { throw new ProviderException_1.default(e); });
        if (typeof result !== "undefined") {
            throw new ProviderException_1.default("The provider returns illegal value.");
        }
    }
    async getUtxos(assetName) {
        const walletProvider = this.checkWalletProvider();
        const utxos = [];
        if (assetName) {
            const unspendableUtxos = await walletProvider.getUnspendableUtxos(assetName)
                .catch((e) => {
                throw new ProviderException_1.default(e);
            });
            const spendableUtxos = await walletProvider.getSpendableUtxos(assetName)
                .catch((e) => {
                throw new ProviderException_1.default(e);
            });
    ...
    async getBalance(assetName) {
        const utxos = await this.getUtxos(assetName);
        return utxos.reduce((balance, utxo) => balance + utxo.corbes, 0);
    }
    async sign(address, dataToSign) {
        if (!this.isLegacyAddress(address)) {
            throw new IllegalArgumentException_1.default("The address is not Legacy Address format.");
        }
    ...
    async buildTransaction(outputs, assetName) {
        if (outputs.length === 0) {
            throw new IllegalArgumentException_1.default("The outputs cannot be empty.");
        }
        return this.createSignedTx(outputs, assetName);
    }
